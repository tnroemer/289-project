...
HAM_IMAGE_DIR = os.path.join(HAM_DATASET_DIR, "HAM10000_images")
...
def find_ham_image_path(image_id):
    local_path = os.path.join(HAM_IMAGE_DIR, image_id + ".jpg")
...
def prepare_ham10000():
    ...
    df["full_image_path"] = df["image_id"].apply(find_ham_image_path)
    df["lesion_image_path"] = df["image_id"].apply(
        lambda x: os.path.join(HAM_LESION_IMAGE_DIR, x + ".jpg")
    )
    ...
    print("Prepared HAM10000 common-label splits")
...
def prepare_pad_ufes20():
    ...
    print("Prepared PAD-UFES-20 common-label image folder")
    print(f"Metadata path: {metadata_path}")
